...
function prepare_data(df, train_test_windows::NTuple{2,Date}, x_var=names(df)[2:end-1], y_var=names(df)[end])
    # format data

    start_date, stop_date = train_test_windows
    
    all_data = filter(row -> row.Date in collect(start_date:Day(1):stop_date), df)
